...
void main() {
  ...
  group('QaReportBuilder', () {
    ...
    test(
      'preserves repeated visits, zero API screens, and request-start association',
      () {
        ...
        expect(data.steps.map((step) => step.route), <String>[
          '/home',
          '/transfer',
          '/home',
        ]);
    ...
    test('orders parallel APIs by start time and insertion order for ties', () {
      ...
      expect(data.steps.single.apis.map((api) => api.path), <String>[
        '/accounts',
        '/tie',
        '/cards',
      ]);
    ...
    test(
      ...
      () {
        ...
        final representation =
            '${reportApi.requestBody}${reportApi.responseBody}';
    ...
    test('applies report and payload limits with omission counts', () {
      ...
      for (var i = 0; i < 3; i++) {
        ...
        events.add(
          _network(
            'a$i',
            base.add(Duration(seconds: i, milliseconds: 1)),
            '/p$i',
      ...
      final data =
          const QaReportBuilder(
            limits: QaReportLimits(
              maxSteps: 2,
              maxApis: 1,
              maxTextPayloadCharacters: 5,
            ),
          ).build(
            events: events,
            notes: '',
      ...
      expect(
        data.steps.first.apis.single.responseBody,
        contains('[preview limited]'),
      );
    });

    test(
      'built report is unchanged when source events change or session clears',
      () {
  ...
  test('text report follows the canonical journey structure', () {
    ...
    for (final expected in <String>[
      'QA REPORT',
      'Generated: 2026-09-06T19:30:22.000Z',
  ...
  group('Phase 7 report semantics', () {
    QaReportData build(
      List<QaEvent> events, {
      String? currentRoute = '/final',
      QaReportLimits limits = const QaReportLimits(),
    }) => QaReportBuilder(limits: limits).build(
      events: events,
      notes: '',
    ...
    test('renders PUSH, POP, and REPLACE as next-navigation semantics', () {
      for (final action in <QaRouteAction>[
        QaRouteAction.push,
        QaRouteAction.pop,
    ...
    test('repeated route visits associate APIs by originating time', () {
      ...
      for (var i = 0; i < 4; i++) {
        events.add(
          _route(
            'r$i',
            base.add(Duration(seconds: i * 2)),
            i == 0 ? null : '/same',
            '/same',
          ),
        );
        events.add(
          _network('a$i', base.add(Duration(seconds: i * 2 + 1)), '/same'),
        );
      }
    ...
    test('equal API timestamps retain source insertion order', () {
      ...
      expect(report.steps.single.apis.map((api) => api.path), <String>[
        '/first',
        '/second',
        '/third',
      ]);
    ...
    test(
      'completion after navigation does not move an API from its originating step',
      () {
        final report = build(<QaEvent>[
          _route('home', base, null, '/home'),
          _network(
    ...
    test('success, failure, and cancellation retain distinct data', () {
      final report = build(<QaEvent>[
        _network(
          ...
          base,
          '/home',
          response: const {'result': 'accepted'},
        ),
        _network(
          'failed',
          base.add(const Duration(seconds: 1)),
          '/home',
          outcome: QaNetworkOutcome.failure,
          response: const {'reason': 'declined'},
        ),
        _network(
          'cancelled',
          base.add(const Duration(seconds: 2)),
    ...
    test(
      'maxSteps and maxApis report accurate global and per-step omissions',
      () {
        final report = build(
          <QaEvent>[
            _route('one', base, null, '/one'),
            ...
            _network(
              'two-a',
              base.add(const Duration(seconds: 1, milliseconds: 1)),
              '/two',
            ),
            _network(
              'two-b',
              base.add(const Duration(seconds: 1, milliseconds: 2)),
              '/two',
            ),
            _route(
              'three',
            ...
            _network(
              'three-a',
              base.add(const Duration(seconds: 2, milliseconds: 1)),
              '/three',
            ),
          ],
          currentRoute: '/three',
          limits: const QaReportLimits(maxSteps: 2, maxApis: 2),
        );
    ...
    test(
      'preview truncation is explicit and collections are deeply snapshot-safe',
      () {
        final payload = <String, Object?>{'value': 'abcdefghij'};
        final source = <QaEvent>[
          _network('api', base, '/home', response: payload),
        ];
        final report = build(
          source,
          currentRoute: '/home',
          limits: const QaReportLimits(maxTextPayloadCharacters: 8),
        );
        payload['value'] = 'changed';
        source.clear();
        expect(
          report.steps.single.apis.single.responseBody,
          contains('[preview limited]'),
        );
        expect(
          report.steps.single.apis.single.responseBody,
  ...
  group('Phase 7 text rendering', () {
    ...
    test('renders outcomes, bodies, error metadata, and omission notices', () {
      final data =
          const QaReportBuilder(
            limits: QaReportLimits(maxSteps: 1, maxApis: 3),
          ).build(
            events: <QaEvent>[
              _route('one', base, null, '/one'),
    ...
    test('known raw secrets never appear in data or canonical text', () {
      ...
      final inspected =
          '${data.currentRoute}|${data.notes}|${data.steps.expand((step) => step.apis).map((api) => '${api.url}|${api.queryParameters}|${api.requestBody}|${api.responseBody}|${api.errorMessage}').join()}';
      final text = const QaReportTextRenderer().render(data);
...
QaRouteEvent _route(
  ...
  String? from,
  String? to, {
  QaRouteAction action = QaRouteAction.push,
}) => QaRouteEvent(
  id: id,
  timestamp: time,
  action: action,
