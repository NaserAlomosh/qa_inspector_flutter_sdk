...
void main() {
  test('writes exact PNG bytes using only the filename basename', () async {
    final directory = await Directory.systemTemp.createTemp('qa-report-share-test-');
    ...
    expect(sharedPath, '${directory.path}${Platform.pathSeparator}qa_report_20260906_232000.png');
  ...
  test('isolates temporary directory and file write failures', () async {
    final preparationResult = await QaReportFileSharer(
      temporaryDirectoryProvider: () => Future<Directory>.error(StateError('unavailable')),
    ).share(
      bytes: Uint8List(0),
      filename: 'report.png',
      sharePositionOrigin: const Rect.fromLTWH(1, 1, 10, 10),
    );
    final writeResult = await QaReportFileSharer(
      temporaryDirectoryProvider: () async => Directory('/missing/qa-report-directory'),
    ).share(
      bytes: Uint8List(0),
      filename: 'report.png',
      sharePositionOrigin: const Rect.fromLTWH(1, 1, 10, 10),
