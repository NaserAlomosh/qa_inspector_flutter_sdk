...
void main() {
  testWidgets('exports repeatable non-empty PNG bytes and filename', (
    tester,
  ) async {
  ...
  testWidgets('PNG-facing widget receives only masked secret values', (
    tester,
  ) async {
    ...
    final report = _data(
      steps: <QaReportStep>[
        _step(1, '/secure', <QaReportApi>[
          _api('/secure', QaNetworkOutcome.success),
        ]),
      ],
    );
    await tester.pumpWidget(
  ...
  testWidgets('unsafe report height returns a controlled failure', (
    tester,
  ) async {
    late BuildContext context;
    await tester.pumpWidget(
  ...
  testWidgets(
    'report document remains outside the active viewport while rendering',
    (tester) async {
      late BuildContext context;
      var taps = 0;
      await tester.pumpWidget(
            ...
            builder: (value) {
              ...
              return Scaffold(
                body: GestureDetector(
                  onTap: () => taps++,
                  child: const SizedBox.expand(key: Key('interactive-host')),
                ),
              );
            },
          ),
      ...
      expect(
        tester.getTopLeft(reportTitle).dx,
        greaterThan(
  ...
  testWidgets(
    'exports representative multi-step, failed, cancelled, notes, and truncated reports',
    (tester) async {
      late BuildContext context;
      await tester.pumpWidget(
        MaterialApp(
  ...
  testWidgets('a tall report below the configured maximum succeeds', (
    tester,
  ) async {
    late BuildContext context;
    await tester.pumpWidget(
    ...
    final report = _data(
      steps: List<QaReportStep>.generate(
        8,
        (index) => _step(index + 1, '/step-$index', const <QaReportApi>[]),
      ),
    );
    final future = const QaReportImageExporter(
      limits: QaReportLimits(
  ...
  testWidgets(
    'unsafe pixel ratios and missing overlays fail without host exceptions',
    (tester) async {
      late BuildContext context;
      await tester.pumpWidget(
        Builder(
      ...
      for (final ratio in <double>[0, -1, 3.1]) {
        final result = await QaReportImageExporter(
          limits: QaReportLimits(pixelRatio: ratio),
        ).export(context, _data());
        expect(result.isSuccess, isFalse);
        expect(result.errorMessage, contains('unsafe'));
      }
      final result = await const QaReportImageExporter().export(
        context,
        _data(),
      );
      expect(result.isSuccess, isFalse);
  ...
  testWidgets('export consumes a stable snapshot after source mutation', (
    tester,
  ) async {
    late BuildContext context;
    await tester.pumpWidget(
    ...
    final sourceSteps = <QaReportStep>[
      _step(1, '/original', const <QaReportApi>[]),
    ];
...
QaReportData _data({String notes = '', List<QaReportStep>? steps}) =>
    QaReportData(
      generatedAt: DateTime.utc(2026, 9, 6, 19, 30, 22),
      ...
      totalApis:
          steps?.fold<int>(0, (total, step) => total + step.apis.length) ?? 0,
      failedApis:
          steps
              ?.expand((step) => step.apis)
              .where((api) => api.isFailure)
              .length ??
          0,
      omittedSteps: 0,
      omittedApis: 0,
...
QaReportStep _step(int number, String route, List<QaReportApi> apis) =>
    QaReportStep(
      number: number,
      route: route,
      screen: route,
...
QaReportApi _api(
  ...
  statusCode: outcome == QaNetworkOutcome.success
      ? 200
