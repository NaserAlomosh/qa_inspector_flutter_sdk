...
void main() {
  group('QaNetworkInterceptor', () {
      ...
      () async {
        ...
        final adapter = _FakeAdapter((options) {
          ...
          return _jsonResponse(
            <String, Object?>{'accessToken': 'response-token', 'result': true},
        ...
        final response = await dio.get<Map<String, dynamic>>(
          '/users',
          queryParameters: <String, Object?>{'otp': 'query-secret', 'page': 2},
          options: Options(
    ...
    test(
      'uses custom sensitive keys everywhere and stores no raw secrets',
      () async {
        final controller = _controller();
        final dio = _dio(
          controller,
    ...
    test(
      'marks cancellation and leaves cancellation behavior unchanged',
      () async {
        final controller = _controller();
    ...
    test(
      'handles parallel requests and identifies request-start chronology',
      () async {
        final controller = _controller();
        ...
        final dio = _dio(
          ...
          adapter: _FakeAdapter(
            (options) =>
                options.path.endsWith('/first') ? first.future : second.future,
          ),
        );
        ...
        final secondEvent = events.singleWhere(
    ...
    test('bounds payloads and omits binary content', () async {
      ...
      await dio.post<void>(
        '/large',
        data: <String, Object?>{'large': List<String>.filled(100, 'x').join()},
      );
      await dio.post<Uint8List>(
        '/binary',
        data: Uint8List.fromList(<int>[4, 5]),
      );
    ...
    test(
      'captures FormData and MultipartFile metadata without finalizing it',
      () async {
        final controller = _controller();
        final formData = FormData.fromMap(<String, Object?>{
          'username': 'tester',
          'password': 'form-password',
          'document': MultipartFile.fromBytes(<int>[
            1,
            2,
            3,
        ...
        final dio = _dio(
          controller,
          adapter: _FakeAdapter(
            (_) => _jsonResponse(<String, Object?>{'ok': true}),
          ),
        );
        ...
        expect(
          _map(files.single)['content'],
          contains('binary content omitted'),
        );
        expect(
          formData.isFinalized,
          isTrue,
          reason: 'Dio alone finalizes FormData',
        );
        controller.dispose();
      },
    );
    ...
    test(
      'supports multiple Dio instances and prevents duplicate registration',
      () async {
        final controller = _controller();
    ...
    test(
      'malformed inspection metadata does not affect host outcomes',
      () async {
        final controller = _controller();
    ...
    test(
      'clear prevents an in-flight ghost event and permits new requests',
      () async {
        final controller = _controller();
        ...
        expect(
          controller.events.whereType<QaNetworkEvent>().single.path,
          '/new',
        );
        controller.dispose();
      },
