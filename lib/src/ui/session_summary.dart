...
class SessionSummary extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final failed = apis.where((event) => event.outcome == QaNetworkOutcome.failure).length;
    final routes = events.whereType<QaRouteEvent>().length;
    return Semantics(
      label: 'Session summary. $routes screens, ${apis.length} APIs, $failed failed.',
      child: Container(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: <Widget>[
          Text('CURRENT ROUTE', style: Theme.of(context).textTheme.labelMedium?.copyWith(color: QaColors.slate)),
          const SizedBox(height: 3),
          Directionality(textDirection: TextDirection.ltr, child: Text(formatRoute(currentRoute), maxLines: 1, overflow: TextOverflow.ellipsis, style: Theme.of(context).textTheme.titleMedium)),
          const SizedBox(height: 12),
          Row(children: <Widget>[
            _Metric(label: 'Screens', value: '$routes'),
            _Metric(label: 'APIs', value: '${apis.length}'),
            _Metric(label: 'Failed', value: '$failed', color: failed > 0 ? QaColors.failure : null),
...
class _Metric extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Expanded(child: Column(children: <Widget>[
    ...
    Text(label, style: Theme.of(context).textTheme.labelMedium?.copyWith(color: QaColors.slate)),
  ]));
}
